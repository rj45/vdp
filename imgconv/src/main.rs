...
use oklab::{srgb_to_oklab, oklab_to_srgb, Oklab, Rgb};
...
fn cli() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() <= 1 {
        ...
        println!("Tilemap size: {}x{}", config.tilemap_width, config.tilemap_height);
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" => {
                ...
                println!("  --palette-hex FILE       Output palette hex file (default: rtl/palette.hex)");
                println!("  --tiles-hex FILE         Output tiles hex file (default: rtl/tiles.hex)");
                ...
                println!("  --tilemap-size WIDTHxHEIGHT Tilemap dimensions in tiles (default: 32x32)");
                ...
                println!("  --dither-factor FLOAT    Error scaling factor for dithering (default: 0.75)");
...
impl ImageConverter {
    ...
    pub fn convert(&self) -> Result<TilemapData, Box<dyn std::error::Error>> {
        ...
        let (tile_palette_assignments, max_tile_error) = self.assign_palettes(&raw_tiles, &palettes)?;
        println!("max_tile_error: {}", max_tile_error / self.config.tile_width as f32 / self.config.tile_height as f32);
        
        // Quantize tiles
        let (quantized_tiles, dither_error) = self.quantize_tiles(&raw_tiles, &palettes, &tile_palette_assignments)?;
    ...
    /// Extract tiles from the image
    fn extract_tiles(&self, img: &image::DynamicImage) -> Result<Vec<Vec<Oklab>>, Box<dyn std::error::Error>> {
        let tile_size = (self.config.tile_width * self.config.tile_height) as usize;
        let mut tiles = Vec::with_capacity((self.config.tilemap_width * self.config.tilemap_height) as usize);
        ...
        for (x, y, pixel) in img.pixels() {
            ...
            if tiles[tile_index].len() <= pixel_index {
                tiles[tile_index].resize(tile_size, Oklab { l: 0.0, a: 0.0, b: 0.0 });
            }
    ...
    fn generate_palettes(&self, tiles: &[Vec<Oklab>]) -> Result<Vec<Palette>, Box<dyn std::error::Error>> {
        let tile_size = (self.config.tile_width * self.config.tile_height) as usize;
        ...
        palettes.sort_by(|a, b| {
            let a_avg_l = a.colors.iter().map(|color| color.color.l).sum::<f32>() / a.colors.len() as f32;
            let b_avg_l = b.colors.iter().map(|color| color.color.l).sum::<f32>() / b.colors.len() as f32;
    ...
    fn reduce_colors(&self, color_frequencies: Vec<ColorFrequency>) -> Result<Vec<ColorFrequency>, Box<dyn std::error::Error>> {
    ...
    /// Assign palettes to tiles
    fn assign_palettes(&self, tiles: &[Vec<Oklab>], palettes: &[Palette]) 
        -> Result<(Vec<usize>, f32), Box<dyn std::error::Error>> {
        
        let mut tile_palette = Vec::with_capacity(
            (self.config.tilemap_width * self.config.tilemap_height) as usize
        );
    ...
    fn quantize_tiles(&self, tiles: &[Vec<Oklab>], palettes: &[Palette], tile_palette_assignments: &[usize])
        -> Result<(Vec<Vec<u16>>, Vec<Vec<Oklab>>), Box<dyn std::error::Error>> {
        
        let tile_size = (self.config.tile_width * self.config.tile_height) as usize;
        let pixels_per_chunk = 4; // 4 pixels per u16 (4 bits per pixel)
        let chunks_per_tile = (tile_size + pixels_per_chunk - 1) / pixels_per_chunk;
        ...
        if self.config.dithering {
            ...
            for _ in 0..total_pixels {
                dither_error.push(Oklab { l: 0.0, a: 0.0, b: 0.0 });
            }
        }
            ...
            for ty in 0..self.config.tile_height {
                for x in 0..self.config.tilemap_width {
                    ...
                    for tx in 0..self.config.tile_width {
                        ...
                        if self.config.dithering {
                            color = Oklab {
                                l: color.l + dither_error[gy * img_width + gx].l,
                                a: color.a + dither_error[gy * img_width + gx].a,
                                b: color.b + dither_error[gy * img_width + gx].b,
                            };
                        }
        ...
        Ok((quantized_tiles, dither_error.chunks(tiles.len()).map(|c| c.to_vec()).collect()))
    }
    
    /// Apply Sierra dithering algorithm to distribute quantization error
    fn apply_sierra_dithering(&self, error: &mut [Oklab], original: Oklab, quantized: Oklab, x: usize, y: usize, width: usize) {
        let img_height = (self.config.tilemap_height * self.config.tile_height) as usize;
        let diff = Oklab {
            l: ((original.l - quantized.l) / 32.0) * self.config.dither_factor,
            a: ((original.a - quantized.a) / 32.0) * self.config.dither_factor,
            b: ((original.b - quantized.b) / 32.0) * self.config.dither_factor,
        };
    ...
    fn generate_tilemap(&self, tile_palette_assignments: &[usize]) -> Result<Vec<u16>, Box<dyn std::error::Error>> {
    ...
    fn write_tiles_file(&self, quantized_tiles: &[Vec<u16>]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for y in 0..self.config.tilemap_height {
            for ty in 0..self.config.tile_height {
                for x in 0..self.config.tilemap_width {
                    ...
                    let row_start = (ty as usize * chunks_per_row) as usize;
            ...
            for x in 0..self.config.tilemap_width {
                ...
                for (chunk_idx, color) in quantized_tiles[tile_index].iter().enumerate() {
                    ...
                    for pixel_offset in 0..pixels_per_chunk {
                        if base_i + pixel_offset >= (self.config.tile_width * self.config.tile_height) as usize {
                            break;
                        }
                        ...
                        if color_index < palette.colors.len() {
                            ...
                            out_img.put_pixel(
                                (x * self.config.tile_width + pixel_x as u32) as u32,
                                (y * self.config.tile_height + pixel_y as u32) as u32,
    ...
    fn create_tilemap_data(
        ...
        let tiles: Vec<Tile> = raw_tiles.into_iter().zip(quantized_tiles.into_iter())
            .map(|(pixels, quantized)| {
                Tile { pixels, quantized }
            })
            .collect();
        
        // Create tilemap entries
        let tilemap_entries: Vec<TilemapEntry> = tilemap.into_iter().enumerate()
            .map(|(i, raw_value)| {
                TilemapEntry {
    ...
    fn write_json_file(&self, path: &str, data: &TilemapData) -> Result<(), Box<dyn std::error::Error>> {
